...
export default function DashStoreKeeperSendStock() {
  ...
  useEffect(() => {
        const fetchData = async () => {
          try {
            const storeIdRes = await fetch(`/api/store/getstore/${currentUser.id}`);
            const storeIdData = await storeIdRes.json();
            if (storeIdRes.ok) {
              setShopId(storeIdData.store[0]);
              console.log(storeIdData);
              const productsRes = await fetch(
                `/api/store-item/getstoreitems/${storeIdData.store.storeId}`
              );
  ...
  const handleSubmit = async (e) => {
    ...
    console.log(sendItemId, formData.shopId, selectedProduct.itemId);
    try {
      const res = await fetch(
        `/api/store-item/senditem/${sendItemId}/${formData.shopId}/${selectedProduct.itemId}`,
        {
          method: "PUT",
          headers: {
...
//   //   try {
//   //     const res = await fetch(
//   //       `/api/store-item/senditem/${sendItemId}/${formData.shopId}/${selectedProduct.id}`,
//   //       {
//   //         method: "PUT",
//   //         headers: {
//   //           "Content-Type": "application/json",
...
//     } catch (error) {
//       console.log(error.message);
//       setCreateUserError("Failed to send stock");
//       setCreateLoding(false);
//     }
//   };
